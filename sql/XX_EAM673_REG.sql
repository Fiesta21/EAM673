...
DECLARE
    ...
    g_cust_file_name        CONSTANT VARCHAR2( 32 )  := 'XXPHA_EAM673_PKG.main';
    g_cust_short_name       CONSTANT VARCHAR2( 16 )  := 'XXPHA_EAM673';
    ...
    g_value_set_name1       CONSTANT VARCHAR2( 32 )  := 'XXPHA_EAM673_PERIOD';        -- Набор значений "Период с", "Период по"
    g_value_set_name2       CONSTANT VARCHAR2( 32 )  := 'XXPHA_HR_OU_SECURED';        -- Набор значений "Операционная единица"
    g_value_set_name3       CONSTANT VARCHAR2( 32 )  := 'XXPHA_EAM673_ORG';           -- Набор значений "Организация"


    TYPE XXPHA_EAM673_PROCESS_TYPE IS TABLE OF VARCHAR2( 256 ) INDEX BY BINARY_INTEGER;

    t_perms_values XXPHA_EAM673_PROCESS_TYPE;                                                                  -- Список полномочий по умолчанию
...
BEGIN
    ...
    -- 2.3) Создание зависимого набора значений "Организация"
    XXPHA_CREATE_CONC.bild_table_value_set( p_value_set_name        => g_value_set_name3
                                            , p_description         => 'Организация'
                                            , p_table               => 'apps.XXPHA_ORGANIZATIONS_V v'
                                            , p_value_column_name   => 'v.organization_name'
                                            , p_value_column_type   => 'C'
                                            , p_value_column_size   => 25
                                            , p_meaning_column_name => NULL
                                            , p_meaning_column_type => NULL
                                            , p_meaning_column_size => NULL
                                            , p_id_column_name      => 'v.organization_id'
                                            , p_id_column_type      => 'N'
                                            , p_id_column_size      => 10
                                            , p_enable_longlist     => 'N'
                                            , p_where_order_by      => 'WHERE ' || CHR( 10 ) ||
                                                                       '    1 = 1 ' || CHR( 10 ) ||
                                                                       '    and v.OPERATING_UNIT = :$FLEX$.XXPHA_HR_OU_SECURED.id '
                                            , p_additional_columns  => NULL
                                            , p_maximum_size        => 200
                                          );
    ...
    --3.3) Добавление параметра 'Организация'
    XXPHA_CREATE_CONC.add_parametr( p_parameter               => 'P_ORG'
                                    , p_description           => 'Организация'
                                    , p_value_set             => g_value_set_name3
                                    , p_default_type          => NULL
                                    , p_default_value         => NULL
                                    , p_required              => 'N'
                                    , p_display               => 'Y'
